"""
This module provides a command-line interface (CLI) for executing batch inference
using the YOLOv8 object detection system.

The script acts as a wrapper around the `InferencePipeline`, offering:
1. CLI Argument Parsing: Allows users to override default configurations for
   input/output directories, model checkpoints, processing device (CPU/CUDA/MPS),
   and worker parallelism.
2. Batch Orchestration: Initializes the inference environment and triggers the
   processing of image and video datasets.
3. Summary Reporting: Prints a structured summary of performance metrics,
   including success rates, total detections, average confidence, and runtime.
4. MLflow Control: Provides an option to toggle experiment tracking directly
...
def main():
    ...
    try:
        ...
        if batch_metrics:
            ...
            print(
                f"✓ Metrics file: {os.path.join(pipeline.output_dir, 'batch_metrics.json')}"
            )

        # Exit with appropriate code
        exit_code = (
            0 if batch_metrics and batch_metrics["failed_inferences"] == 0 else 1
        )
        sys.exit(exit_code)
